import 'package:dilidili/utils/iterable_ext.dart';
import 'package:dilidili/utils/parse_string.dart';
...
class Top {
  ...
  Top.fromJson(Map<String, dynamic> json) {
    try {
      imgUrls =
          (json['result'] as List?)?.map((e) => TopImage.fromJson(e)).toList();
    } catch (_) {}
  }
}
  ...
  TopImage.fromJson(Map<String, dynamic> json) {
    ...
    try {
      ...
      if (image['location'] case String locStr when (locStr.isNotEmpty)) {
        final location =
            locStr.split('-').skip(1).take(2).map(num.parse).toList();
